...
$cls_user = new User();    
//$usernameShop = isset($_REQUEST['username']) ? $_REQUEST['username'] : '';

// Khóa bí m?t - du?c c?p b?i pay.net.vn
$SECURE_SECRET = "932864918e25572d4cd873caaec2a7dc";
// L?y giá tr? chu?i mã hóa d? li?u
...
if ($paynet_status == '0'){
	...
	if ($md5HashData == $paygate_md5hash){	
		...
		if($uid>0 && $amount>0){
			...
			mysql_query('' . 'insert into payments set payment_status=\'Completed\', advid=\''.$uid.'\', payment_amount = \''.$amount.'\',text = \''.$md5HashData.'\', method=\'Card\' ');	     
		}
